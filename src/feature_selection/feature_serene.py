...
def feature_selections(features):
    ...
    t_test_reduced = np.delete(X_test, idx, axis=1)
    
    return X_train_reduced, t_test_reduced
...
def logistic_regression_features():
    ...
    X_new.shape
    features = lr.coef_
    X_new, x_test = feature_selections(features)
  
    return logistic_regression(X_new, X_test, y_train, y_test)
...
def support_vector_machine_features():
    ...
    X_new = model.transform(X_train)
    features =  svm.coef_
    X_train, x_test = feature_selections(features)
